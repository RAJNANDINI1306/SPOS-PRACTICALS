...
public class PASS {
  ...
  public static void main(String Args[])throws IOException {
    BufferedReader b1 = new BufferedReader(
      new FileReader("intermediatecode.txt")
    );
    ...
    FileWriter f1 = new FileWriter("PASS_2.txt");
    ...
    while ((s = b2.readLine()) != null) {
      String word[] = s.split("\t\t\t");
      if (word.length >= 2) {  // Check if split produced at least 2 elements
    ...
    while ((s = b1.readLine()) != null) {
      if (s.substring(1, 6).compareToIgnoreCase("IS,00") == 0) {
        ...
        if (s.charAt(8 + offset) == 'S') {
          f1.write(
            symSymbol.get(
              Integer.parseInt(s.substring(10 + offset, s.length() - 1))
            ) +
            "\n"
          );
        } else {
