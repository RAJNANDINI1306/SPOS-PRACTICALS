...
public class macroPass2 {
  ...
  public static void main(String[] Args) throws IOException {
    ...
    BufferedReader b4 = new BufferedReader(new FileReader("ala.txt"));
    ...
    while ((s = b3.readLine()) != null) mdt.addElement(s);
    while ((s = b4.readLine()) != null) kpdt.addElement(s);
    ...
    while ((s = b1.readLine()) != null) {
      ...
      if (macroNameHash.containsKey(b1Split[0])) {
        ...
        while (i <= actualParams.length) {
          ...
          aptab.put(
            aptabInverse.get(
              initializedParams[0].substring(1, initializedParams[0].length())
            ),
        ...
        while (mdt.get(i).compareToIgnoreCase("MEND") != 0) {
          ...
          for (int j = 0; j < mdt.get(i).length(); j++) {
            if (mdt.get(i).charAt(j) == '#') f1.write(
              aptab.get(Integer.parseInt("" + mdt.get(i).charAt(++j)))
            ); else f1.write(mdt.get(i).charAt(j));
          }
          f1.write("\n");
          i++;
        }
